...
if args.output is None:
    ...
    base = os.path.basename(args.reports).rstrip('/\\')
    base = base if len(base)>0 else \
            os.path.basename(os.path.dirname(args.reports)).rstrip('/\\')
    print(f'base: "{base}"')
    out_filepath = os.path.join(outdir, f'{base}-ctakes.jsonl')
else:
